...
def calc_metrics(requests: list[RequestStats]) -> dict:
    ...
    # Derive accurate kb_per_token from current model + scheduler config.
    # Falls back to per-model lookup (or GLM-5 default) when ConfigManager
    # isn't populated yet (external callers without a running scheduler).
    try:
        ...
        if model_info is not None and sched_config is not None:
            kb_per_token = kv_bytes_per_full_token(model_info, sched_config) / 1024
        elif model_info is not None and model_info.model_path:
